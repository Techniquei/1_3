...
void Array::addFurniture(float heightSet, float lengthSet, float widthSet,
                  float coordZSet, float coordYSet,float coordXSet, string nameSet, string materialSet){
    this->size++;
    this->array.push_back(Furniture(heightSet, lengthSet, widthSet,
                                           coordZSet, coordYSet, coordXSet, nameSet, materialSet));
}

void Array::addAppliances(float heightSet, float lengthSet, float widthSet,
                  float coordZSet, float coordYSet,float coordXSet, string nameSet){
    this->size++;
    this->array.push_back(Appliances(heightSet, lengthSet, widthSet,
                                           coordZSet, coordYSet, coordXSet, nameSet));
}
...
bool Array::arrayCheck() {
    cout<<"\n\nCHECK STARTED";
    int counter=0;    
    for(int i=0; i<this->size; i++) {
        for(int j=i; j<this->size; j++) {
            cout<<"\n: "<<i<<j;
            if((this->indexSearch(j).getMaterial()==this->indexSearch(i).getMaterial())&&(this->indexSearch(i).getMaterial()!="NoN")){
                ...
                if((((((this->indexSearch(j).getCoordX()-this->indexSearch(i).getCoordX()-
                       this->indexSearch(i).getWidth())<0)&&
                        ...
                        ((this->indexSearch(i).getMaterial()=="NoN")&&(this->indexSearch(i).getCoordZ()>0))){
                            counter++;
